...
class StorageClient(storage.Client):
    ...
    def upload_from_string(self, destination_path: str, data: bytes | str, content_type: str = "plain/text", timeout: int = 60):
        ...
        """
        destination_bucket = destination_path.split("/")[0]
        destination_file_path = destination_path.split("/")[1:]
        delim = "/"
...
class CloudClient:
    ...
    with open(os.path.join(config_values.root_dir, 'creds.json'), 'r+') as f:
...
if __name__ == "__main__":
    ...
    cloud_client.storage_client.upload_from_string("rawcrawlerdatalake/test/test.txt","test", "text/plain")